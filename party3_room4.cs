...
public class Portal : IScriptV2
{
	...
	{
		if (GetFieldsetVar("Party5", "stage5") == "clear")
		{
		...
		if (IsLeader)
		{
			...
			{
				Message(character, "The leader of the party has entered the <Sealed Room>.");
			});
			
			MapPacket.PlayPortalSE(chr);
			ChangeMap(920010500, "st00");
		}
		else
		{
			if (UserCount(920010500) >= 1)
			{
				MapPacket.PlayPortalSE(chr);
				ChangeMap(920010500, "st00");
			}
